...
class MCX:
    def __init__(self, hostname):
        ...
        self.pathCert = os.path.abspath("mcx.cert.pem")
        # self.pathCert = os.path.abspath("license.lic")
        print("path", self.pathCert)
        self.inputCmd = 'root/ManipulatorControl/hostInTool/Position/input'
        self.swithCmd = 'root/ManipulatorControl/autoMotionGenerator/userFrames/switch01/toggle'
        self.currentPoseCmd = 'root/ManipulatorControl/actualToolCoordinates'
        self.ethercatParam = 'root/Ethercat/Robot/EL2024 4K. Dig. Ausgang 24V, 2A/Channel 1'
        self.buttonState = 'root/ManipulatorControl/dummyBool'
        self.robotState = "root/Logic/stateCommand"
        self.robotModeCmd = "root/Logic/modeCommand"
        self.watchdogCmd = "root/ManipulatorControl/hostInTool/Position/track/watchdog/enable"
        # self.trackingCmd = "root/ManipulatorControl/hostInTool/Position/track/isTracking"
        self.relativeCmd = "root/ManipulatorControl/hostInTool/Position/track/relative"
        self.enableCmd = "root/ManipulatorControl/hostInTool/Position/track/enable"
        self.filterBetaCmd = "root/ManipulatorControl/hostInTool/Position/filter/beta"
        self.filterOmegaCmd = "root/ManipulatorControl/hostInTool/Position/filter/omega"
        self.STATE = 0
        self.connect()
    ...
    def connect(self):
        ...
        try:
            ...
            self.setParameterByPath(self.enableCmd, 0)
            self.setParameterByPath(self.watchdogCmd, 1)
    ...
    def MoveToStartPoint(self):
        ...
        self.point = []
        self.start_position = [0.55, 0.0, 0.079, radians(90), radians(0), radians(180)]

        self.point.append(Waypoint(self.start_position))
    ...
    def writePoint(self, point: list):
        print("Writing point {0}".format([point[0] + self.start_position[0], point[1] + self.start_position[1], point[2] + self.start_position[2]] + [0, 0, 0]))

        self.req.setParameter(
            self.inputCmd, [round(point[0], 4) + self.start_position[0], round(point[1], 4) + self.start_position[1], round(point[2], 4) + self.start_position[2]]).get()
        
        print("Writing sensor {0}".format([round(point[0], 4), round(point[1], 4), round(point[2], 4)] + [0, 0, 0]))

        if self.STATE == 0:
            self.setParameterByPath(self.robotModeCmd, 4)
            self.STATE = 1
            time.sleep(1)
            self.setParameterByPath(self.watchdogCmd, 0)
            self.setParameterByPath(self.enableCmd, 1)

        
    def setParameterByPath(self, path, value):
        self.req.setParameter(path, value).get()
        
    def checkConstraine(self, point):
        startPose = self.start_position
    ...
    def automatic(self):
        zValue = float(self.z_sensor_control_range[0])
        coordList = []
        start_pos = Waypoint([0.3, 0.114, 0.254, radians(90), radians(0), radians(180)])
        
        coordList.append(Waypoint([0.30075, -0.51904, 0.24786, radians(95.83828480245795), radians(25.511858116418647), radians(180)]))
        coordList.append(Waypoint([0.06856, -0.52441, 0.24786, radians(95.83828480245795), radians(25.511858116418647), radians(180)]))
        coordList.append(Waypoint([0.06856, -0.47395, 0.19726, radians(95.83828480245795), radians(25.511858116418647), radians(180)]))
        coordList.append(Waypoint([0.30075, -0.47201, 0.19726, radians(95.83828480245795), radians(25.511858116418647), radians(180)]))
        coordList.append(Waypoint([0.30075, -0.51904, 0.24786, radians(95.83828480245795), radians(25.511858116418647), radians(180)]))
...
def main():
    try:
        ...
        mcx.stopMotor()
        mcx.setParameterByPath("root/ManipulatorControl/hostInTool/Position/track/enable", 0)
        mcx.setParameterByPath("root/ManipulatorControl/hostInTool/Position/track/watchdog/enable", 1)
